...
class LocalitiesTest < ApplicationSystemTestCase
  ...
  test "should create locality" do
    ...
    check "Active status" if @locality.active_status
    fill_in "Assigned code", with: @locality.assigned_code
    check "Del status" if @locality.del_status
    fill_in "Description", with: @locality.description
    fill_in "Name", with: @locality.name
    fill_in "Suburb code", with: @locality.suburb_code
